...
- FR-3 分页采集：滚动懒加载翻页，--pages 指定页数，1 ≤ pages ≤ 8（FR-3.1），自动停止于末页（FR-3.2）
- FR-4 去重：同一 jobId（无 id 时按 公司+名称）全流程去重（FR-4.1）
- FR-5 详情描述：默认串行抓取职位描述；--no-detail 可跳过（FR-5.1）
...
### NFR
- NFR-1 风控护栏：相邻翻页动作间隔 ≥ 2.5s（±1.5s 抖动），详情抓取 ≥ 0.8s（±1.0s）；
  页数上限 8；护栏参数下调需人工批准
...
- AC-2：pages 越界（0/9）与节拍参数过低被拒绝并给出明确信息
...
- AC-9（人工）：真实检索一次杭州/测试职位可产出非空 CSV（由用户在本地执行并回填证据）
