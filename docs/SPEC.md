...
- FR-8 自动筛选：--job-type/--salary/--experience/--degree 传页面筛选文本，
  自动在筛选面板选择并生效（FR-8.1）；多条件可组合（FR-8.2）；全程无人工干预
...
- D7（2026-09-07 修订，用户要求自动化）筛选自动化：筛选下拉选项为 li[ka="sel-job-rec-{field}-{code}"]，
  label→code 运行时可从 DOM 发现，CLI 传文本即可自动点击（每次筛选点击同样受节拍约束）；
  替代原「人工暂停调筛选后回车」设计。若页面结构变化导致选项不可见，明确报错而不是静默跳过
